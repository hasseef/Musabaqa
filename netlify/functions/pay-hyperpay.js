// Netlify Function: Create HyperPay Checkout (test endpoint)
// Requires env vars: HYPERPAY_ENTITY_ID, HYPERPAY_BEARER (test creds from HyperPay dashboard)
export const config = { path: '/pay-hyperpay' };
export default async (req, context) => {
  try{
    ...
    // Choose entity id per method if needed
    const entityId = process.env.HYPERPAY_ENTITY_ID;
    const bearer = process.env.HYPERPAY_BEARER;
    if(!entityId || !bearer){
    ...
    const body = new URLSearchParams({
      ...
      // Optional: customer.email, billing.street1, city, country, etc.
    });
